...
while (lnum <= inFile_size):
  ...
  if (os.path.isfile('/hadoop/cms/store/user/' + getpass.getuser() + '/' + parts[0].split('/')[1] + '/' + parts[0].split('/')[1] + '_' + parts[0].split('/')[2] + '/*/0000/*.root'):
    if (redoCrab == 2): redoCrab = int(input("Some unmerged files already exist!  Remake them? (1/0)"))
